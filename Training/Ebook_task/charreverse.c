...
Step 4: Check the WHLIE condition, call the function getline(char line[],int maxline) to declare len and
        check len is greater than 0 and len is less than 40,it's true goto step 5 else goto step
...
Step 8: Print the max length line in reverse order by using strrev function in string.
Step 9: STOP.
...
Step 5: STOP.*/
 ...
 int main()
 {
    ...
    if (len > max && len <40)                              //Checks len is greater than max
    {
      max = len;
      copy(longest, line);                        //Copies the line from line to longest
    }
    if (max > 0) /* there was a line */
    printf("\nLongest line is= %s", strrev(longest));      //Prints the longest line in reverse by strrev
    return 0;
 }
...
/*OUTPUT
This ia a reverse of the line
^Z

Longest line is=
 enil eht fo esrever a ai sihT
Process returned 0 (0x0)   execution time : 16.627 s
